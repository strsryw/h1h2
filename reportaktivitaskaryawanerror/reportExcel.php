...
$sheet->mergeCells(1, 0, 1, 10);
$sheet->mergeCells(2, 0, 2, 10);
...
$sheet->write($baris, 10, "Status", $fHd);
...
while (!strrpos($data, "^") === false) {
    ...
    if (trim($tablename) == "gridReportAktivitasKaryawan") {
		...
		$sheet->write($baris, 10, $status, $fLL);
	...
	if($baris=="65535"){
		$sheet = $xls->addWorksheet('Check_Rekap_Absensi_1');
		$sheet->setPaper(5); // legal
		//$sheet->setLandscape();
		$sheet->setPortrait();
		$sheet->setMarginLeft(0.6);
