...
//	Start Excel
	...
	$xls->send("Check-Rekap-Absensi-".namaBulan($bulangaji)."-$tahungaji.xls");
	$fTt = $xls->addFormat();
	$fTt->setBold();
...
//	$sheet->write(baris, kolom, text, format)
	$sheet->write($baris, 0, "LAPORAN CHECK REKAP ABSENSI", $fTt);
...
//	$sheet->mergeCells(awalBaris,awalKolom,akhirBaris,akhirKolom);
	...
	$sheet->write($baris, 3, "Rekap Absen", $fHd);
	$sheet->write($baris, 4, "Non Finger", $fHd);
	$sheet->write($baris, 5, "Penyimpangan", $fHd);
	...
	while (!strrpos($data,"^")===false){
		...
		if(trim($tablename)=="reportabsensi") {
			...
			$sheet->write($baris, 3, $rekapabsen, $fLL);
			$sheet->write($baris, 4, $nonfinger, $fLL);
			$sheet->write($baris, 5, $penyimpangan, $fLL);
