...
if (!empty($mode)) {
	if ($mode == "getData") {
		...
		$reportMerk = "SELECT id,nama,kode FROM newasset.typedetail WHERE nama LIKE '%$NmMerkCari%' ORDER BY nama";
		...
		if ($jumRecord == 0 || $jumRecord == Null) {
			...
			foreach ($rsTampil as $dataMerk) {
				$txtHead = "<tr class='small'>
					...
					<th width='60%' align=\"center\">Nama Type</th>
					<th width='10%' align=\"center\">Kode</th>
					<th width='20%' align=\"center\">Action</th>
					</tr>";
				$txtData .= "
	...
	if ($mode == "saveData") {
		try {
			...
			if ($idMerk == 0 || $idMerk == '') {
				$rsCek = $dbCon->execute("select id from newasset.typedetail where nama='$nmMerk' and kode='" . $kdMerk . "'");
				$jumlahCek = $dbCon->getNumRows($rsCek);

				if ($jumlahCek <= 0) {
					$querisimpan = "INSERT INTO newasset.typedetail(nama,kode) VALUES ('$nmMerk','" . $kdMerk . "')";
					...
					echo "Data Gagal Di Input!!!  Type $nmMerk sudah ada!!!";
				}
			} else {
				$rsCek = $dbCon->execute("select id from newasset.typedetail where nama='$nmMerk' and kode='" . $kdMerk . "'");
				$jumlahCek = $dbCon->getNumRows($rsCek);
				if ($jumlahCek <= 0) {
					$rsUpdatenmMerk = $dbCon->execute("UPDATE newasset.typedetail set nama='$nmMerk', kode='" . $kdMerk . "' WHERE id='$idMerk'");
					if (!$rsUpdatenmMerk) {
					...
					echo "Data Gagal Di Update!!!  Type $nmMerk masih ada !!!";
				}
			}
	...
	if ($mode == 'delData') {
		try {
			$dbCon->execute("BEGIN");
			$idMerk = $_POST['idMerk'];
			$hasil = $dbCon->execute("DELETE FROM newasset.typedetail WHERE id =$idMerk");
			if ($hasil) {
				echo "Success";
			} else {
				echo "Error";
