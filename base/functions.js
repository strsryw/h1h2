...
pesan[33]="No bukti tsb sudah dihapus";
...
function loadGridArray(gridobj,tblName,buffer) {
   ...
   tableData = new Array();
    i=0;
  data =buffer.value;
  	//console.log(gridobj);return
   while (data.indexOf("^")>=0){
     iValue=data.substring(0,data.indexOf("^"));
     data=data.substring(data.indexOf("^")+1);
     tablename = getColumnValue(iValue, 1).trim();
	 //update script load after deldata
	 var rValue=iValue.split("|");
	 var kolId=rValue[1];
	 var deletedId=0;
	 ...
	 if(tablename==tblName) {
		 ...
		 while(iValue.indexOf("|")>=0){
              ...
              iValue=iValue.substring(iValue.indexOf("|")+1);
				...
				if (!firstColumn){
					//update script load after deldata
					//alert(kolId.indexOf("-")>=0)
      				if(kolId.indexOf("-")>=0){deletedId=1;}
		      		
					cellData=[cValue];
					//update script load after deldata
              		//rowData=rowData.concat(cellData);
					if(deletedId!=1){rowData=rowData.concat(cellData);}
				}
			  firstColumn=false;
         }
	   //update script load after deldata
       //tableData[i++]=rowData;
	   //alert(rowData)
	   if(deletedId!=1){tableData[i++]=rowData;}
